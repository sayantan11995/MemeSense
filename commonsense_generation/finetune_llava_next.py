...
prompt_template = """As an AI assistant for social media content moderation, your role is to analyze and prevent the posting of harmful, offensive, hateful, vulgar, misogynistic, or otherwise inappropriate content on public platforms.
...
# dataset = load_dataset("imagefolder", data_dir="/home/student/sayantan/safety_llm/image_data/hg_datasets_formatted_data/" )
dataset = load_dataset("imagefolder", data_dir="/home/student/sayantan/safety_llm/commonsense_labelled_data_new/" )
...
train_dataset = LlavaDataset("../commonsense_labelled_data_new/",  split="train")
val_dataset = LlavaDataset("../commonsense_labelled_data_new/", split="validation")
...
def train_collate_fn(examples):
    ...
    for example in examples:
        ...
        # prompt = f"[INST] <image>\nDescribe the meme and mention whether it contains any harmful content. [\INST] {ground_truth}"
        # prompt = f"[INST] <image>\nDescribe the meme and mention whether it contains any harmful content. [\INST] {ground_truth}"
        # prompt = f"[INST] <image>\nDescribe the meme and assess whether the meme has the potential to be perceived as vulgar, harmful, or unethical using specific commonsense parameters. [\INST] {ground_truth}"
...
def eval_collate_fn(examples):
    ...
    for example in examples:
        ...
        images.append(image)
        # TODO: in the future we can replace this by processor.apply_chat_template
        # prompt = f"[INST] <image>\nDescribe the meme and mention whether it contains any harmful content. [\INST]"
...
model_module.model.save_pretrained("saved_model_detailed_prompt")
